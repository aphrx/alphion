...
import { StyleSheet, Text, View, ScrollView, TextInput, ImageBackground, Keyboard } from 'react-native';
import { TouchableOpacity } from 'react-native-gesture-handler';
import ExerciseTileWithSets from '../components/ExerciseTileWithSets.js';
import { insertExercise, deleteExercises, updateWorkouts, deleteTask } from '../services/Database.js';
...
const EditWorkoutScreen = ({ route, navigation }) => {
    ...
    const checkIfValid = () => {
      for(let i = 0; i < exerciseList.length; i++){
        if(exerciseList[i].exerciseSets == null ||
      ...
      <View style={styles.container}>
        ...
        <View style={styles.tasksWrapper}>
          <View style={styles.newWorkoutTile}>
            <View style={styles.cardContainer}>
                <ImageBackground source={backgrounds[selectedColour]} style={styles.imageBackground} imageStyle={{ borderRadius: 20}}>
                  <View style={styles.imageInner}>
                    <TextInput style={styles.imageText} value={name} placeholder={'New Workout'} onChangeText={(text) => renameWorkout(text)} autoFocus={true}/>
                    <View style={styles.buttons}>
                        <TouchableOpacity onPress={() => handleDeleteWorkout(workoutId)}><FontAwesome5 name={'trash-alt'} style={styles.trashIcon} solid /></TouchableOpacity>
                    </View>
                  </View>
                  <View style={styles.imageOptions}>
                    <TileOptionButton key={selectedColour} opt={0} selected={selectedColour} onPress={onSelectColor}/>
                    <TileOptionButton key={selectedColour+1} opt={1} selected={selectedColour} onPress={onSelectColor}/>
                  </View>
                </ImageBackground>
            </View>
        ...
        <View style={styles.exerciseWrapper}>
        {
            exerciseList.length != 0 ? (
              exerciseList.map(({exerciseId, exerciseName, exerciseMuscle, exerciseSets, exerciseReps}, index) => {
